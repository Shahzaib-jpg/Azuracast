<template>
    ...
    <div class="card">
        ...
        <div class="list-group list-group-flush" v-if="files.length > 0">
            <span v-for="(rowFile1, rowIndex1) in files" v-bind:key="rowIndex1" track-by="id" >
                <div v-if="id=='playlist_2' && rowFile1.mypl"  v-bind:id="'playid_'+rowFile1.id"> 
                <a href="#"  v-on:click.prevent="play({ fileIndex: rowIndex1 },rowFile1.id)" class="list-group-item list-group-item-action flex-column align-items-start"
                    >
...
export default {
    ...
    methods: {
        ...
        play (options,id='') {
           ...
           //console.log(options);
            ...
            return this.getStream().createFileSource(this.file, this, (source) => {
                console.log(source);
                console.log('ob',this.file);
                $(`.list-group-item`).removeClass('active');
                $(`#playid_${this.file.id}`).children('.list-group-item').addClass('active')
        ...
        onEnd () {
            ...
            $(`.list-group-item`).removeClass('active');
            //$(`#playid_${id}`).children('.list-group-item').addClass('active')
        },
       ...
       async fetchPlaylist(){
        ...
        async addToIndex(file,id,cat_id){
         ...
         async addToIndex2(file,id,cat_id,title){
            //alert();
             console.log("file new neww neww");
            console.log(file);
             file.readTaglibMetadata(async (data) => {
                console.log(data);
                console.log("data");
